...
create table if not exists public.moa_friendships (
  ...
  primary key(a,b), check(a<b), check(requester in (a,b))
);
alter table public.moa_friendships enable row level security;
revoke all on public.moa_friendships from public,anon,authenticated;
-- No client can write relationships or read another account's calendar document.
-- Mutations and sanitized calendar reads are available only through this RPC.
...
begin
  ...
  if action='accept' or action='decline' then
    ...
    -- Explicit opt-in only; no private fields, counts, group events, or busy markers.
    select coalesce(jsonb_agg(jsonb_build_object('id',e->>'id','title',e->>'title','date',e->>'date',
      ...
      'allDay',e->'allDay','note',e->>'note','type',e->>'type')),'[]'::jsonb) into result
    from public.calendar_accounts c cross join lateral jsonb_array_elements(c.data->'events') e
    where c.user_id=target and e->>'visibility'='friends' and e->>'type' in ('personal','hobby')
      and e->>'date'<end_day::text and coalesce(e->>'endDate',e->>'date')>=start_day::text;
    return result;
  end if;
